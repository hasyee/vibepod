...
└── pages/
    ├── PodcastSearchPage.tsx     # /search/podcasts?q=
    ├── EpisodeSearchPage.tsx     # /search/episodes?q=
    ├── PodcastEpisodesPage.tsx   # /search/podcasts/:podcastId
...
| `/search/podcasts`            | `PodcastSearchPage`   | Podcast search results (`?q=`)  |
| `/search/podcasts/:podcastId` | `PodcastEpisodesPage` | Podcast detail + episode list   |
| `/search/episodes`            | `EpisodeSearchPage`   | Episode search results (`?q=`)  |

The `SearchBar` in the navbar reads and writes the `?q=` URL param. A `SegmentedControl` (Podcasts / Episodes) switches between `/search/podcasts` and `/search/episodes` while preserving the query.
...
Reads the current route and `?q=` param. On input change, navigates to `/search/podcasts?q=` or `/search/episodes?q=` (preserving mode). A `SegmentedControl` switches mode while keeping the query. Focusing the input when not on a search route navigates to `/search/podcasts`.
...
### `PodcastSearchPage` (`/search/podcasts?q=`)

Reads `?q=` from URL, debounces 400ms, fetches podcasts via `searchPodcasts`. Clicking a card navigates to `/search/podcasts/:id`.

### `EpisodeSearchPage` (`/search/episodes?q=`)

Reads `?q=` from URL, debounces 400ms, fetches episodes via `searchEpisodes`.

### `PodcastEpisodesPage` (`/search/podcasts/:podcastId`)

Fetches podcast metadata (`fetchPodcast`) and episodes (`fetchEpisodes`) in parallel. Shows a large header with artwork, title, author, genre, Subscribe button, and description. The Subscribe button toggles the subscription via `SubscriptionContext`. Back button returns to `/search/podcasts?q=` (preserving the query).

### `SubscriptionsPage` (`/subscriptions`)

Reads `subscriptions` from `SubscriptionContext`. Displays a grid of `PodcastCard`s. Clicking navigates to `/search/podcasts/:id`.
